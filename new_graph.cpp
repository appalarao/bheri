...
void multi_point()
{
	...
	while(!q.empty())
	{
		...
		while(temp!=NULL)
		{
			if(mat[val][temp->data]>temp->data+temp->wt)
		}
	}
}
*/
...
queue<int> q;
int source;
cout<<"enter source between 0 "<<n<<endl;
cin>>source;
node *temp;
dist[source]=0;
parent[source]=-1;
...
try
{
	while(!q.empty())
	{
		val=findmin(dist,visit);
		if(val!=-1)
		{
			cout<<"\n value is "<<val<<" and it's neibhors are --> \n";
			temp=g[val];
			visit[val]=1;
			while(temp)
			{
				cout<< temp->data <<" wt is "<<temp->wt<<endl;
				if(!visit[temp->data])
				{
					q.push(temp->data);
					if(dist[temp->data]>(temp->wt+dist[val]))
					{
...
{
	cout<<"exception is "<<expn<<endl;
}
catch(...)
{
	cout<<"default exception"<<endl;
}
cout<<"single point distance from node "<<source <<" to rest is "<<endl;
for(i=1;i<n;i++)
{
	cout<<dist[i]<<"\t";
}
